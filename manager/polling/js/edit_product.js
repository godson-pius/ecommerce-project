...
(function () {
    document.getElementById("edit_product").addEventListener("submit", function (e) {
        e.preventDefault();
        ...
        fetch(`polling/php/edit?product_id=${product_id}`, {
            method : "post",
            body : form_inputs
        }).then(r => r).then(r => r.text()).then(r => {
            if (r === "true") {
                toastr.success("Product edited successfully!","Product Edited");
                document.getElementById("submit_btn").innerHTML = "<i class='icon-note'></i> Edit Product";
